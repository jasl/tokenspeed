...
def sparse_mla_fp8_cache_cuda(
    ...
    if k_split > 16:
        raise ValueError(f"k_split must be <= 16, got {k_split}")
    if k_split > 1:
        num_tokens, num_heads = q_arg.shape[0], q_arg.shape[1]
        required = num_tokens * num_heads * k_split * (512 + 2)
        if partials is None:
            partials = torch.empty(required, dtype=torch.float32, device=q_arg.device)
        else:
            if partials.dtype != torch.float32:
                raise TypeError(f"partials must be float32, got {partials.dtype}")
            if partials.device != q_arg.device:
                raise ValueError("partials must be on the q device")
            if partials.numel() < required:
                raise ValueError(
                    f"partials scratch too small: have {partials.numel()} "
                    f"floats, need at least {required}"
                )
            partials = partials.contiguous()
    else:
        partials = torch.empty(0, dtype=torch.float32, device=q_arg.device)
